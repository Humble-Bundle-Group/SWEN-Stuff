...
public class StockManager
{
    ...
    public StockLibrary loadLibrary(StockLibrary sl)
    {
            ...
            try
            {
                while ((part = br.readLine()) != null) {
                    if (part.compareTo("enddata") != 0) {
                        ...
                        Part p = new Part(partData[1], partData[3], Integer.parseInt(partData[5]), Float.parseFloat(partData[7]), partData[9], partData[11]);
                        sl.addPart(p);
                    }
                }
                sl.printStock();
        ...
        {
            System.out.println("Error loading previous stock data found.");
        }
        return sl;
    }
